...
class Account extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            ['billing_vatId','match','pattern'=>'/^((AT)?U[0-9]{8}|(BE)?0[0-9]{9}|(BG)?[0-9]{9,10}|(CY)?[0-9]{8}L|(CZ)?[0-9]{8,10}|(DE)?[0-9]{9}|(DK)?[0-9]{8}|(EE)?[0-9]{9}|(EL|GR)?[0-9]{9}|(ES)?[0-9A-Z][0-9]{7}[0-9A-Z]|(FI)?[0-9]{8}|(FR)?[0-9A-Z]{2}[0-9]{9}|(GB)?([0-9]{9}([0-9]{3})?|[A-Z]{2}[0-9]{3})|(HU)?[0-9]{8}|(IE)?[0-9]S[0-9]{5}L|(IT)?[0-9]{11}|(LT)?([0-9]{9}|[0-9]{12})|(LU)?[0-9]{8}|(LV)?[0-9]{11}|(MT)?[0-9]{8}|(NL)?[0-9]{9}B[0-9]{2}|(PL)?[0-9]{10}|(PT)?[0-9]{9}|(RO)?[0-9]{2,10}|(SE)?[0-9]{12}|(SI)?[0-9]{8}|(SK)?[0-9]{10}|[0-9]{10,10})$/', 'message' => 'Ihre Umsatzsteuer-ID enthält Fehler.'],
           // ['billing_vatId','match','pattern'=>'/^((AT)?U[0-9]{8}|(BE)?0[0-9]{9}|(BG)?[0-9]{9,10}|(CY)?[0-9]{8}L|(CZ)?[0-9]{8,10}|(DE)?[0-9]{9}|(DK)?[0-9]{8}|(EE)?[0-9]{9}|(EL|GR)?[0-9]{9}|(ES)?[0-9A-Z][0-9]{7}[0-9A-Z]|(FI)?[0-9]{8}|(FR)?[0-9A-Z]{2}[0-9]{9}|(GB)?([0-9]{9}([0-9]{3})?|[A-Z]{2}[0-9]{3})|(HU)?[0-9]{8}|(IE)?[0-9]S[0-9]{5}L|(IT)?[0-9]{11}|(LT)?([0-9]{9}|[0-9]{12})|(LU)?[0-9]{8}|(LV)?[0-9]{11}|(MT)?[0-9]{8}|(NL)?[0-9]{9}B[0-9]{2}|(PL)?[0-9]{10}|(PT)?[0-9]{9}|(RO)?[0-9]{2,10}|(SE)?[0-9]{12}|(SI)?[0-9]{8}|(SK)?[0-9]{10}|[0-9]{10,11})$/', 'message' => 'Ihre Umsatzsteuer-ID enthält Fehler.'],
         //   ['billing_zipcode','match','pattern'=>'/(?!([089])\1{4})\d{5}/', 'message' => 'Ihre Postleitzahl enthält Fehler.'],
         //   ['shipping_zipcode','match','pattern'=>'/(?!([089])\1{4})\d{5}/', 'message' => 'Ihre Postleitzahl enthält Fehler.'],
          	[['personal_email'], 'email'],
          	[['personal_email'], 'validateUsername'],
          	[['personal_email'], 'unique'],
			[['personal_customer_type', 'personal_salutation','personal_firstname', 'personal_lastname', 'personal_email', 'personal_passwordConfirmation', 'personal_phone', 'billing_company', 'billing_department', 'billing_vatId', 'billing_street', 'billing_additionalAddressLine1', 'billing_zipcode', 'billing_city', 'billing_country', 'billing_shippingAddress', 'shipping_salutation', 'shipping_company', 'shipping_department', 'shipping_firstname', 'shipping_lastname', 'shipping_street', 'shipping_additionalAddressLine1', 'shipping_zipcode', 'shipping_city', 'shipping_country', 'authKey'], 'string', 'max' => 255],
    ...
    {
		return static::findOne(['accountid' => $id]);
    }
